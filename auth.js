...
function updateHeaderAccount() { const header = document.querySelector('.header'); if (!header || header.querySelector('.auth-links')) return; const session = getSession(), links = document.createElement('div'), cartLink = header.querySelector('.order-link'); if (cartLink && !session) cartLink.style.display = 'none'; links.className = 'auth-links'; if (session) { const signOutLink = document.createElement('a'); signOutLink.className = 'auth-link'; signOutLink.href = '#'; signOutLink.textContent = 'Sign out'; signOutLink.addEventListener('click', event => { event.preventDefault(); signOut(); }); links.append(signOutLink); } else { links.innerHTML = '<a class="auth-link" href="sign-in.html">Sign in</a><a class="auth-link auth-signup" href="sign-up.html">Sign up</a>'; } const menu = header.querySelector('.menu-toggle'); if (menu) header.insertBefore(links, menu); else header.append(links); }
